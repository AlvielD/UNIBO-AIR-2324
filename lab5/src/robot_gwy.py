...
"""
import rospy as ros
from environment_pkg.srv import BoxPos
...
class RobotGateway():
    def __init__(self):
        ...
        # Services for getting box position
        self.box_position_srvs = {}
        ros.wait_for_service('BOX1/get_position')
        self.box_position_srvs.update({
            'box1': ros.ServiceProxy('BOX1/get_position', BoxPos)
        })
        ros.wait_for_service('BOX2/get_position')
        self.box_position_srvs.update({
            'box2': ros.ServiceProxy('BOX2/get_position', BoxPos)
        })
        ros.wait_for_service('BOX3/get_position')
        self.box_position_srvs.update({
            'box3': ros.ServiceProxy('BOX3/get_position', BoxPos)
        })
    ...
    # TODO: Implement by suing the gazebo services explained by the tutor
    def get_box_position(self, box):
        """
        Virtual sensor to read the current (x,y) position of a given box in global frame
        In simulation, this uses an ad-hoc service implemented in Gazebo
        ...
        """
        pos = self.box_position_srvs[box](True)

        return (pos.x_pos, pos.y_pos, 0.0, 0.5)